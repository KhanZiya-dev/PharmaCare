...
def extract_medicines_from_image(image_path: str) -> list[str]:
    """
    Extracts medicine names from an image using the google.genai SDK.
    """
    ...
    for api_key in api_keys:
        if time.time() - total_start > 170:
            logger.warning("Aborting vision API call early to prevent 180s timeout on frontend.")
            break
        ...
        try:
            ...
            if "503" in err_str or "unavailable" in err_str or "404" in err_str or "not_found" in err_str:
                # Even for 503, continue to try the next key, as another key might be routed differently or have higher quota
                continue
                
            continue

    if last_error:
        raise ValueError(f"AI Vision processing failed: {str(last_error)}")
